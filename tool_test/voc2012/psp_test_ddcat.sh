...
$PYTHON -u tool_test/voc2012/test_voc_psp_ddcat.py \
  --config=${config} \
  2>&1 | tee ${result_dir}/testing_no_attack/test-$now.log


$PYTHON -u tool_test/voc2012/test_voc_psp_ddcat.py \
  --config=${config} --attack --attack_name ${attack_name} --attack_iterations ${attack_iterations} --attack_epsilon ${attack_epsilon} --attack_alpha ${attack_alpha} \
